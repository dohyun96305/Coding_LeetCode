WITH TEMP AS (
    SELECT 
        A.*, 
        B.TEAM_NAME AS FROM_PASS_TEAM, 
        C.TEAM_NAME AS TO_PASS_TEAM, 
...
GROUP_TEAM AS (
    SELECT 
        FROM_PASS_TEAM AS TEAM_NAME, 
        (TIME_RANK - TEAM_RANK) AS TEAM_STREAK_GROUP,
...
SELECT 
    TEAM_NAME, 
    MAX(COUNT_STREAK) AS LONGEST_STREAK
FROM GROUP_TEAM
GROUP BY TEAM_NAME
ORDER BY TEAM_NAME;
